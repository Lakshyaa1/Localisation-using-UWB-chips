...
# CONFIG
# ============================================================
INPUT_JSON = "(100,200)_C_08_01_2026.json"
OUTPUT_JSON = "uwb_positions_nlls_bounded.json"
...
# ============================================================
W_MIN = 0.1      # minimum anchor influence
W_MAX = 5.0      # maximum anchor influence
...
# ============================================================
# WEIGHTED NLLS WITH BOUNDED + SMOOTHED IRLS
# ============================================================
def trilaterate_nlls_weighted(anchor_pos, distances, x0, prev_weights=None):
    ...
    for _ in range(NLLS_ITERS):
        ...
        # ---- Update weights (bounded + smoothed) ----
        for mac, (ax, ay) in anchor_pos.items():
            d_est = math.hypot(x_est - ax, y_est - ay)
            r = d_est - distances[mac]

            w_raw = 1.0 / (1e-3 + abs(r))
            w_clamped = min(max(w_raw, W_MIN), W_MAX)

            w_prev = weights.get(mac, 1.0)
            weights[mac] = ALPHA * w_clamped + (1 - ALPHA) * w_prev
...
def main():
    ...
    print("\nUWB POSITIONING (JSON → LLS → NLLS + BOUNDED IRLS)")
    print("------------------------------------------------\n")
